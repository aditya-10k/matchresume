...
2. STRICT NO-ASTERISK RULE (MANDATORY):
   - Under NO circumstances should you use the asterisk character (*) anywhere in your output.
   - Do NOT use asterisks for bolding (e.g. do NOT write **word**).
   - Do NOT use asterisks for italics (e.g. do NOT write *word*).
   - Do NOT use asterisks for bullet points (e.g. do NOT write * item or * **heading**).
   - For bullet lists, use a standard hyphen followed by a space (- item) or numbers (1., 2.).
   - For headings, use plain text or markdown hashes (### Heading).
3. EXECUTIVE QUALITY: Provide structured, clean, well-formatted answers.
   - Highlight project names, quantifiable metrics (percentages, latency, users), and architecture decisions without using asterisks.
   - Keep answers punchy, confident, and direct.
4. CONVERSATIONAL FOLLOW-UPS: Generate 2-3 actionable, high-value follow-up questions without asterisks.

Return valid JSON matching this schema:
{
  "answer": "Complete answer directly addressing the query with NO asterisks...",
  "suggested_followups": [
    ...
    def answer_query(
        ...
        if not user_resumes and not raw_chunks:
            return CareerQueryAnswer(
                query=query,
                answer="### No Resume Uploaded Yet\n\nI do not have any verified background to analyze. Please upload your resume PDF in the Resume Vault or Studio so I can map your projects, skills, and experience!",
                evidence=[],
                suggested_followups=[
        ...
        for c in raw_chunks:
            ...
            clean_content = strip_asterisks(c.content)
        ...
        if not evidence_texts and user_resumes:
            for r in user_resumes[:2]:
                snippet = strip_asterisks(r.raw_text[:800]) if r.raw_text else ""
                if snippet:
...
Remember: DO NOT use asterisks (*) anywhere in your response.
...
"""
        ...
        try:
            ...
            answer_text = strip_asterisks(raw_answer_text)
            raw_followups = list(data.get("suggested_followups", []))
            clean_followups = [strip_asterisks(f) for f in raw_followups if f]
            ...
            return CareerQueryAnswer(
                ...
                suggested_followups=clean_followups or [
            ...
            return CareerQueryAnswer(
                query=query,
                answer=strip_asterisks(fallback_answer),
                evidence=evidence_list,
                suggested_followups=[
    ...
    def _generate_extractive_fallback(self, query: str, evidence: List[CareerEvidenceSnippet]) -> str:
        ...
        for ev in evidence[:4]:
            lines.append(f"- {ev.section.capitalize()} ({ev.source_resume}): {strip_asterisks(ev.content.strip()[:240])}...")
        
        lines.append("\n> Grounded directly in your indexed resume chunks.")
        return strip_asterisks("\n\n".join(lines))
