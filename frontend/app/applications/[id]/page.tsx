...
export default function ApplicationDetailPage({
  ...
  return (
    <div className="relative mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      {/* Top Header Card */}
      <div
        className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 rounded-3xl border bg-black/50 p-6 shadow-2xl backdrop-blur-2xl transition-all"
      ...
      >
        <div>
          <div className="flex items-center gap-2">
            ...
            {application.company && (
              <span className="text-xs font-semibold text-zinc-400">@ {application.company}</span>
            )}
          </div>
      ...
      <div className="mt-8 grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div
          className="rounded-3xl border bg-black/40 p-6 shadow-2xl backdrop-blur-xl flex flex-col items-center justify-center text-center"
          style={{ borderColor: `${selectedModel.colors.primary}25` }}
        >
          <MatchGauge score={recommendation.match_score} size={170} />

          <div className="mt-5 text-center">
            <span className="text-[11px] font-medium text-zinc-400">Recommended Resume Starting Point:</span>
        ...
        <div
          className="lg:col-span-2 rounded-3xl border bg-black/40 p-6 shadow-2xl backdrop-blur-xl flex flex-col justify-between"
          style={{ borderColor: `${selectedModel.colors.primary}25` }}
        >
          <div>
            <h3 className="text-xs font-bold uppercase tracking-wider text-zinc-400 border-b border-white/10 pb-2">
            ...
            {/* Strengths Grid */}
            <div className="mt-5">
              <span className="text-[11px] font-semibold text-emerald-400 flex items-center gap-1.5 mb-2">
              ...
              <div className="flex flex-wrap gap-1.5">
                {recommendation.strengths.map((skill) => (
                  <span
                    key={skill}
                    className="inline-flex items-center gap-1 rounded-full border border-emerald-500/20 bg-emerald-950/25 px-3 py-1 text-xs font-medium text-emerald-300 shadow-sm"
                  >
              ...
              <div className="mt-4">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {recommendation.gaps.map((skill) => (
                    <span
                      key={skill}
                      className="inline-flex items-center gap-1 rounded-full border border-amber-500/20 bg-amber-950/25 px-3 py-1 text-xs font-medium text-amber-300 shadow-sm"
                    >
          ...
          <p className="mt-4 text-[10px] text-zinc-500 border-t border-white/10 pt-2">
      ...
      <div
        className="mt-6 rounded-3xl border bg-black/35 p-6 shadow-xl backdrop-blur-xl"
      ...
      >
        ...
        <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-6">
          <div>
            <h4 className="text-xs font-semibold text-white mb-2">Mandatory Technical Requirements</h4>
            <div className="flex flex-wrap gap-1.5">
          ...
          <div>
            <h4 className="text-xs font-semibold text-white mb-2">Nice-To-Have Competencies</h4>
            <div className="flex flex-wrap gap-1.5">
        ...
        {requirements.responsibilities.length > 0 && (
          <div className="mt-5 border-t border-white/10 pt-4">
            <h4 className="text-xs font-semibold text-white mb-2">Key Role Responsibilities</h4>
        ...
        >
          ...
          <ChevronDown
            className={`h-4 w-4 text-zinc-400 transition-transform duration-200 ${
              showEvidence ? "rotate-180 text-white" : ""
            }`}
          />
        </button>
            ...
            >
              {relevant_evidence.length === 0 ? (
                ...
                relevant_evidence.map((chunk, idx) => (
                  <div
                    key={idx}
                    className="rounded-2xl border border-white/10 bg-black/50 p-3.5 text-xs font-mono text-zinc-300"
                  >
                    <div className="flex items-center justify-between text-[10px] text-zinc-500 border-b border-white/5 pb-1.5 mb-2">
